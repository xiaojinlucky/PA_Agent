...
- 2026-07-24 `WO-RUN-06` 与动态杠杆后续代码已按本机实际执行链闭合到离线验收：`WorkerStore` v4 持久化 `risk_runtime_state` 和七天 OKX 账单连续扫描边界；新增风险与调杠杆在券商写入前都刷新 `account.totalEq`/账单并执行 `>=50%` 回撤停止，下一笔按最新总权益、真实止损和容量重新定仓。杠杆计划必须在有界策略网格逐点读取容量、由真实监督授权，并只经 Controller→Worker→Service→OkxAdapter 写入和回读；容量不单调或最大杠杆仍不足时继续阻断，不截断数量。当前生产控制库仍为 schema v1、Worker PID 536 仍是旧进程，本段只代表代码与离线测试完成，尚未迁移或加载；迁移、旧 uncertain 处置和重启后现场验收未完成。
...
- 2026-07-24 已重新读取用户提供的网页版 GPT 共享对话并核对正式资金规则：资金转入、转出和账户划转不能计为交易盈亏，也不能虚假抬高资金调整后的历史最高净值；资金变化本身不是交易信号。当前代码已把最近七天 OKX 账单连续扫描边界、USDT `type=1/subType=11/12` 外部转入转出、`account.totalEq` 高水位、50% 新增风险停止和人工 Worker 重锚接入控制库；旧转账自然滑出七天窗口不会误停，连续扫描真正中断满七天才失败关闭。Funding 账户充值/提现、非 USDT 时点换算和七天以上停机补链仍未实现；生产库尚未迁移到 v4，因此不能把代码通过写成已上线。
...
- 2026-07-24 Campaign `52a4f507-a8f8-4975-b0bf-ddddf9ff901c` 当前仍为 `active / 10m / 20 / extreme_aggressive`；现场只读回查为空仓、空普通单、八类算法单均为 0、无活动 execution、无 NEW_RISK 租约。WinSW `PAAgentExecutionWorker` PID 536 心跳健康，但启动早于本轮动态杠杆/风险运行态代码，尚未加载新代码。旧 execution `8c0f83ab-fc6b-589e-8967-c4bd8f538015` 已 `canceled`，其 `uncertain` submit 命令继续作为审计证据保留；必须在控制库 v1→v4 持锁迁移后，用账户身份绑定的只读券商证据确认未写入，不能盲目重提或直接解除。
