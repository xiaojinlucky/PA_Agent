...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页已完成源码与提交级证据。界面符号提交 `c509516a7c57469d49995e95f36a060fe5667ca4` 的 CI、候选资产和 16 图逐张复核通过。文档提交 `6b81574013340c162ac9e83e8f3636084f8499dd` 的 CI run `30519576042` 与候选 run `30520056543` 也全绿，但下载复核发现候选五层能力索引仍绑定旧 SHA，并错误保留发布链三层未完成状态，因此该候选已拒绝。当前本地已让候选索引绑定目标 SHA 和实际 JUnit/全新安装证据，压缩后再次逐文件核对 ZIP、索引和语义；8 个红色反例全部转绿，非 live 2245 项通过、0 失败，独立复核无 P0/P1/P2。仍须新 SHA 的 CI、候选资产和 16 图逐张复核关闭 D 阶段。稳定发布还受阶段 A 真实运行态和正式快捷方式桌面矩阵阻塞。
...
- D 阶段的字体缺字已由 `c509516a` 关闭；其 16 图最终复核通过。后续 `6b81574` 候选又暴露能力索引未绑定候选 SHA，证明工作流仍可带着旧机器状态误绿。当前修复会在压缩前生成 SHA 绑定索引，并在压缩后复核 ZIP 全文件、索引哈希、2245 项测试门和完整安装合同；尚待新提交级证据。
