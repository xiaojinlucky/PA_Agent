...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。归档修复提交 `e8354044a5b261c65a1eef772f58163da7887f22` 的 CI run `30507586401` 已全绿，确定性门为 2229 项通过、0 失败/错误、1 项宿主机条件跳过。候选 Release run `30507852571` 已越过仓库扫描、离线入口、2229 项发布测试、证据索引和真实源码归档，但全新安装发现 PATH 中多个 `git.exe` 被安装脚本当成一个命令。当前已让脚本明确选择首个可执行 Git；回归会在临时 PATH 中自行放置两个 Git 并确认首项，避免依赖宿主机环境。独立审查发现的非确定性 P2 已关闭，本地非 live 2230 项通过、0 失败；本机真实全新 Python 3.12 安装、37 个提示词、两个入口、默认交易关闭、完整 SHA、临时快捷方式和受控卸载也均通过。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行。之后仍要完成候选资产核验和真实快捷方式桌面矩阵。
...
- D 归档修复已由 `e8354044a5b261c65a1eef772f58163da7887f22` 的 CI run `30507586401` 证明全绿。候选 run `30507852571` 也实际通过源码归档门，随后在全新安装中因 `Get-Command git` 返回三个程序而失败，离屏证据和资产上传未执行。多 Git 回归先红后绿，安装脚本现只选择 PATH 中首个应用；测试自行构造两个 Git 并断言发现顺序，独立审查的非确定性 P2 已关闭。用独立归档解压目录完成真实新装后，两个入口、自检、37 个提示词、默认交易关闭、完整 SHA 和临时快捷方式均通过，受控卸载只移除 `.venv` 与快捷方式并保留源码。本地非 live 2230 项通过、0 失败；提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行，当前不能冒充候选资产或稳定 Release。
