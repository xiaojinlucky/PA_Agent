...
- 私有仓库：`Jinqingchang/PA_Agent`；本轮起始 HEAD 为 `f08b9e696da638895c52ffc0a8b7f3562395766d`，当前实盘闭环仍是未提交工作区改动。
- 用户已终止全部前端重设计；本轮只增加交易所必需的功能配置/状态窗口，不做视觉重构。
- 已实现默认关闭的执行闭环：严格分析记录落盘、白名单计划构建、SQLite 强账本、会话/环境硬门、重启对账、Longbridge 双账户路由、OKX 现货/永续、入场/部分成交/保护/主动离场、账户资金与盈亏回写。
- Longbridge 行情数据源仍仅使用 `QuoteContext`；交易执行模块按综合账户和日内账户分别创建独立上下文。日内账户只在提交前明确最大数量不足时回退综合账户，任何网络/认证/未知/已提交状态均不回退。
- OKX 不硬编码黄金；动态规格测试覆盖 `XAUT-USDT`、`XAU-USDT-SWAP`、`BTC-USDT`、`BTC-USDT-SWAP`。现货与永续有独立数量、保护和盈亏语义。
- 所有真实写操作要求 `PA_AGENT_LIVE_TRADING_ENABLED=true`、当前进程会话确认；OKX Live 还要求 `OKX_LIVE_ENABLED=true`。当前两个开关保持关闭，没有发送真实订单。
