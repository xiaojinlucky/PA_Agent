...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。UTC 归档与中文字体证据提交 `4f9543478b1bac12d293fd3a8b3296180e5cfbd7` 的 CI run `30516280889` 已全绿：2235 项通过、0 失败/错误、1 项宿主机条件跳过；实时健康检查 7 项全部因外部条件跳过、0 失败。下载的 6 文件 CI 证据、候选 run `30516712766` 的四资产、上海时区逐字节重建、全新 Python 3.12 安装、37 个提示词、两个入口和默认交易关闭均通过。但独立审查发现加载/分析中的 `↻` 不受固定 Noto 字体支持，显示为带叉方框，旧机器门只检查中文而误绿，因此该候选已拒绝。当前本地将加载符号改为受支持的 `…`，把全部界面必需符号加入固定字体覆盖和元数据硬门；两个红色反例、受影响图片和本地 16 图机器合同已转绿，CI/候选数量门提高到 2237。仍须由新 SHA 的 CI、下载候选和 16 图逐张复核关闭阶段 D，再处理阶段 A 真实运行态和正式快捷方式桌面矩阵。
...
- D 阶段的 `4f95434` CI、候选 workflow、下载资产和跨时区源码重建均通过，但独立图像审查发现 `↻` 缺字，证明旧字体合同仍会误绿；该候选未被记为最终证据。当前本地已用受支持符号替换，并新增全部界面符号覆盖硬门和元数据拒绝测试，仍须绑定新 SHA 重跑。能力索引继续返回 `stable_release_ready=false`。
