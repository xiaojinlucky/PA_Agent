...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。路径误报修复提交 `025becfcc1bb2aee2d09b6aac01db92c8c65b9d6` 的 CI run `30513486939` 已全绿：2232 项通过、0 失败/错误、1 项宿主机条件跳过，证据绑定完整 SHA、Python 3.12.10 和 91 行依赖快照；实时健康检查 7 项全部因外部条件跳过、0 失败。下载后加严复核又发现两份 JUnit 的跳过文本分别带 1 处和 7 处 GitHub Runner 绝对路径，因此该 CI 证据还不能用于发布，也未触发候选 workflow。当前新增原子化 JUnit 脱敏：只替换绝对路径，保留用例名称、跳过原因和测试数量；真实 Runner 证据复现前后分别保持 2232/0/0/1 与 7/0/0/7，脱敏后的 6 文件目录通过路径与密钥扫描。本地发布定向测试和非 live 2233 项均通过、0 失败；仍须让该修复对应 SHA 重新通过提交级 CI、候选 workflow 和下载资产复核。之后仍要完成真实快捷方式桌面矩阵。
...
- D 路径误报修复提交 `025becfcc1bb2aee2d09b6aac01db92c8c65b9d6` 的 CI run `30513486939` 已全绿，但下载后复核发现宿主机条件跳过会把 GitHub Runner 绝对路径写入 JUnit；因此没有把“工作流绿色”冒充“证据可发布”，也未触发候选。当前 CI 会在上传前原子脱敏离线与实时两份 JUnit，并扫描整个证据目录；候选 workflow 也在组装前脱敏自己的 JUnit。真实 CI 证据复现共替换 8 处路径，测试结果合同完全不变，脱敏后路径与密钥扫描均通过。本地非 live 2233 项通过、0 失败。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行，当前不能冒充候选资产或稳定 Release。
