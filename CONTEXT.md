...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。Qt 生命周期与 PowerShell 5.1 编码修复已由 `3221fe88769a844bb4a2c0a55681f98ed2446122` 闭环：CI run `30506024918` 全绿，确定性门为 2227 项通过、0 失败/错误、1 项宿主机条件跳过。候选 Release run `30506362460` 又通过仓库密钥扫描、离线入口、2227 项发布测试和证据索引，但在源码归档门正确拒绝了由 21 个 `.gitkeep` 带入的根运行态目录。首轮未锚定的 `records export-ignore` 又被对抗审查证明会误删 `pa_agent/records`，该证据已撤回；当前规则只锚定仓库根四目录，归档保留 7 个记录模块，并让扫描器拒绝任何运行态 `.gitkeep`。新增后本地非 live 为 2229 项通过、0 失败/错误/跳过；该修复对应的新 SHA CI 与候选 workflow 尚未完成。之后仍要完成全新 Python 3.12 安装和真实快捷方式桌面矩阵。
...
- D 发布链的 Qt 生命周期和 PowerShell 5.1 编码修复已由 `3221fe88769a844bb4a2c0a55681f98ed2446122` 的 CI run `30506024918` 证明全绿。候选 run `30506362460` 的发布测试也全绿，随后源码 ZIP 扫描正确拒绝四个只含 `.gitkeep` 的根运行态目录，未进入全新安装或资产上传。对抗审查又拦下首轮会误删 `pa_agent/records` 的无锚规则；最终规则只排除根目录，真实 `git archive` 保留 37 个提示词、两个入口、完整 SHA 和全部 7 个记录模块，禁止项为 0。扫描器对四种运行态 `.gitkeep` 也已失败关闭；63 项定向和 2229 项非 live 全量通过。该归档修复仍待新 SHA 的提交级 CI 与候选 workflow，不能冒充干净源码安装或稳定 Release。
