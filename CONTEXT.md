...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。JUnit 脱敏提交 `6bdc3b2afcfe8f739d7beb872a56d92946700fea` 的 CI run `30514568230` 已全绿：2233 项通过、0 失败/错误、1 项宿主机条件跳过；实时健康检查 7 项全部因外部条件跳过、0 失败。下载的 6 文件 CI 证据绑定该 SHA、Python 3.12.10 和 91 行依赖快照，8 处 Runner 路径均已脱敏，路径与密钥扫描通过。候选 run `30514904187` 也已全绿并上传四个资产；解包源码和证据扫描、清单、哈希、全新安装、JUnit 与 16 张图的机器合同均通过。但独立下载复核发现源码 ZIP 的 565 个条目内容完全相同、时间戳却随主机时区相差 8 小时，导致稳定发布脚本在上海主机重建时哈希不同；第一张下载图还因 Runner 缺中文字体而大量显示方框。当前源码归档强制 UTC，上海时区重建已与旧候选逐字节一致；离屏证据固定下载并校验 Noto Sans CJK SC 字体，验证器新增中文覆盖硬门，本机 Noto 截图已人工确认中文清晰。本地新增回归与发布定向测试通过；仍须让该修复对应 SHA 通过 2235 项全量、CI、候选 workflow 和下载后 16 图逐张复核。之后仍要完成真实快捷方式桌面矩阵。
...
- D JUnit 脱敏提交 `6bdc3b2afcfe8f739d7beb872a56d92946700fea` 的 CI 与候选 workflow 均全绿，下载 CI 证据和候选四资产也已完成路径、密钥、清单、哈希、解包、JUnit、全新安装及机器桌面合同复核。稳定复核仍拦下两个误绿：`git archive` 的 ZIP 时间戳受主机时区影响，且 Runner 无中文字体时图片会出现方框。当前归档子进程固定 UTC；候选离屏环境从 Noto 官方仓库固定提交下载 16,437,364 字节字体并校验 SHA-256，截图脚本要求核心中文字符全部受支持，桌面验证器也拒绝缺失覆盖的元数据。本机上海时区重建源码 ZIP 已与旧候选逐字节一致，Noto 正常图已人工确认。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 重跑，当前不能冒充最终候选资产或稳定 Release。
