...
- 阶段 A 的正式栅栏协议已由 `bef8e986787d8c5055af32a63516859365a6239e` 发布并安装。切换器提交 `2d81d450db780ec6e751ce8ca538dc63df239f6e` 的 CI run `30562647041` 全绿，但生产只读 `--check` 发现该版把“7 条异常复用命令”误写成“全部历史命令只能有 7 条”，因此安全拒绝且未改数据库。新鲜 OKX Demo 私有真相仍为空仓、空普通单和空八类算法单；生产 Worker 保持停止，控制库仍为 schema v4、风险停止仍开启。当前修复只冻结异常部分为 2 组、组大小 3 与 4、合计 7 条且全为 `submit`，其他完整合法终态历史原样封存；intent、档案、回执、恢复和 v5 空队列均绑定同一计划摘要。48 项切换测试和全仓非 live 2343 项均通过、0 失败/错误/跳过；独立复审 P0/P1/P2 均无。修复 SHA 尚待绿色 CI，之前禁止生产切换。
