...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。D 阶段的单一版本真值、两个离线入口、Windows 源码安装/卸载、发布包严格合同、脱敏五层证据和候选 workflow 已完成本地实现；最终非 live 为 2224 项通过、0 失败/错误/跳过。D 尚待提交级 CI、提交后 `git archive` 和全新 Python 3.12 安装，真实桌面矩阵仍须用户从正式快捷方式验收。
...
- D 阶段本地发布合同测试 52 项、扩展定向回归 305 项、最终非 live 2224 项均通过，失败/错误/跳过均为 0；工作流测试数门同步为 2224。发布清单额外敏感字段的红色反例先真实失败，严格字段合同和上传集合扫描落地后转绿。当前仍是未提交工作区，不能冒充提交级 CI、干净源码安装或稳定 Release。
