...
- 阶段 A 的正式栅栏协议已由提交 `bef8e986787d8c5055af32a63516859365a6239e` 发布并安装；CI run `30549842331` 全绿，远端确定性门为 2295 项、0 失败/错误、1 项跳过。生产 Worker 保持停止，控制库仍为 schema v4、风险停止仍开启。当前安全切换器只接受已授权的 7 条历史命令和 2 组历史租约复用：计划摘要决定唯一切换编号，intent 与 active 栅栏在同一全局锁内耐久提交，v4 与 execution 账本被只读封存并做哈希/ACL/完整逻辑复核，活动 v5 队列从空开始且原样保留风险停止；中断只允许前向恢复，不自动回滚 v5。45 项切换测试和全仓非 live 2340 项均通过、0 失败/错误/跳过；独立复审未发现 P0/P1。目标 SHA 尚待取得绿色 CI，随后才允许执行生产切换。
