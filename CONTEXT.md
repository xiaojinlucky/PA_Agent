...
- 阶段 A 发现正式 v4 库有 2 组历史授权复用，普通 v5 迁移按设计失败关闭。用户已授权显式安全切换；实现改为两阶段。栅栏性能与回执修复提交 `f1884e7b57d330e4d55c3512fdb3f778571d6220` 的 CI run `30544534498` 已全绿：2272 项、0 失败、1 项宿主条件跳过，实时健康检查 7 项均因外部条件跳过、0 失败。安装前关闭旧 GUI 后，只读硬门发现它曾写下严格的旧版 `generation=0/inactive/auto-bootstrap` 状态，缺少当前实例标记；普通写入和直接安装均按设计拒绝。当前修复把一次恢复绑定到源状态哈希、目标 Git SHA、实例与耐久恢复意图；只有同时持有 GUI、Worker、Campaign、数据库四重锁并反复核对源码的正式入口可执行，schema、回执和最终收口均受 SQLite 写事务保护，旧公开安装方法已禁用。47 项栅栏测试和精确提交集合的非 live 2295 项均通过、0 失败；三轮复审最终无 P0/P1。尚待新提交及该 SHA 的绿色 CI，生产数据库仍为 v4 且未修改。
