...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。多 Git 安装修复提交 `c5db9e866a3783b9a280bc7d32d1df533c159780` 的 CI run `30509205694` 已全绿：2230 项通过、0 失败/错误、1 项宿主机条件跳过，证据绑定完整 SHA、Python 3.12.10 和 91 行依赖快照；实时健康检查 7 项、0 失败、5 项外部条件跳过，诚实标为部分可用。候选 Release run `30509503103` 已通过仓库扫描、发布测试、证据索引、真实源码归档和全新 Python 3.12 安装/快捷方式检查，随后因 100% 缩放图命名为 `scale-1-*`、验证合同要求 `scale-1p0-*` 而失败，资产未上传。命名合同回归先红后绿，本机按同流程生成并验证 16 张图和 16 份元数据，发布相关 65 项与非 live 2231 项均通过、0 失败；对应发布 SHA 的提交级 CI 和候选 workflow 仍待运行。之后仍要完成候选资产核验和真实快捷方式桌面矩阵。
...
- D 多 Git 修复已由 `c5db9e866a3783b9a280bc7d32d1df533c159780` 的 CI run `30509205694` 和候选 run `30509503103` 共同证明：全新 Python 3.12 安装、两个入口、自检、37 个提示词、默认交易关闭、完整 SHA 和临时快捷方式均通过；受控卸载仍只认此前的本机验证，候选在运行卸载命令前已失败。候选生成全部 16 张离屏图，但 100% 缩放文件名与验证合同不一致，集合校验失败且资产未上传。工作流现显式映射 `1p0/1p25/1p5`；新回归会直接执行工作流真实命名块并与验证器的六个缩放文件名逐一比较。本机同流程的 16 张图、16 份元数据和发布验证器均通过，非 live 2231 项通过、0 失败。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行，当前不能冒充候选资产或稳定 Release。
