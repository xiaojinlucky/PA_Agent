...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。D 发布链实现已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b` 推送；Qt 生命周期修复已由 `ed8c52ee61fa2ff379b561f554774c3ac4da619e` 推送，CI run `30504823117` 已完整运行到 100%，证明原 Qt 底层崩溃关闭。该 run 随后因 Windows PowerShell 5.1 把三份无 BOM 中文发布脚本按旧编码读坏而出现 3 项失败；当前已为三份脚本补齐 UTF-8 BOM，并把该合同写入测试。PowerShell 5.1/7 均可解析，最终非 live 为 2227 项通过、0 失败/错误/跳过；编码修复仍待新 SHA 的提交级 CI。之后还要完成候选 workflow、`git archive`、全新 Python 3.12 安装和真实快捷方式桌面矩阵。
...
- D 发布链主体已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b` 推送；Qt 生命周期修复 `ed8c52ee61fa2ff379b561f554774c3ac4da619e` 的 CI 已跑完整套件，不再出现底层访问冲突，但因 Windows PowerShell 5.1 的脚本编码兼容问题出现 3 项发布测试失败。当前三份发布脚本均已改为 UTF-8 BOM，现有解析测试新增明确字节合同；PowerShell 5.1/7 双版本解析、61 项发布/离线/E2E 定向测试和 2227 项非 live 全量均通过。编码修复对应的新 SHA 提交级 CI 尚未完成，不能冒充干净源码安装或稳定 Release。
