...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller、C 阶段原生 PyQt6 新页和 D 阶段候选发布链均已完成源码与提交级证据。最后一个代码提交 `c509516a7c57469d49995e95f36a060fe5667ca4` 的 CI run `30518459269` 与候选 run `30518850754` 全绿：非 live 2237 项通过、0 失败；下载的 CI 证据、四个候选资产、上海时区逐字节重建、全新 Python 3.12 安装、37 个提示词、两个入口、默认交易关闭和扫描均通过。16 张最终候选图已逐张打开复核，全部无缺字、截断或重叠；字体元数据同时证明核心中文和全部界面符号受支持。能力索引仍诚实返回 `stable_release_ready=false`，因为阶段 A 真实运行态和正式快捷方式桌面矩阵尚未通过，稳定 tag/Release 不得创建。
...
- D 阶段的首批字体候选因 `↻` 缺字被拒绝；修复后的 `c509516a` 已用受支持的 `…` 替换，并把全部界面符号纳入固定字体和元数据硬门。该 SHA 的 CI、候选 workflow、下载资产、跨时区重建和 16 图逐张复核均通过，阶段 D 候选证据已关闭；能力索引继续返回 `stable_release_ready=false`。
