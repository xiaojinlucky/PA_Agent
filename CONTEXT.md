...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。缩放命名修复提交 `151be1dfb406d65469d0e231963892c7d2c93529` 的 CI run `30510592355` 已全绿：2231 项通过、0 失败/错误、1 项宿主机条件跳过，证据绑定完整 SHA、Python 3.12.10 和 91 行依赖快照；实时健康检查 7 项、0 失败、6 项外部条件跳过，诚实标为部分可用。候选 Release run `30510973235` 已通过仓库扫描、发布测试、证据索引、真实源码归档、全新 Python 3.12 安装/快捷方式检查和 16 张离屏桌面证据生成；证据组装随后把依赖清单中的固定 HTTPS Git 地址误认成 Windows 私有路径，资产未上传。红灯证明根因是大小写不敏感的盘符规则把 `https://` 尾部的 `s://` 当成 `S:\`；当前修复增加盘符左边界，同时仍拒绝真实盘符、`file:///` 和用户目录。完整测试报告中的密钥反例参数名也改成安全 ID，测试内容和密钥扫描门未放宽。本地发布相关 66 项、非 live 2232 项和候选同款完整证据目录扫描均通过、0 失败；仍须让该修复对应 SHA 通过提交级 CI、候选 workflow 和下载资产复核。之后仍要完成真实快捷方式桌面矩阵。
...
- D 缩放命名修复已由 `151be1dfb406d65469d0e231963892c7d2c93529` 的 CI run `30510592355` 和候选 run `30510973235` 证明：工作流按 `1p0/1p25/1p5` 生成并验证全部 16 张离屏图；真实源码归档、全新 Python 3.12 安装、两个入口、自检、37 个提示词、默认交易关闭、完整 SHA 和临时快捷方式也均通过。候选在后续证据组装时因 HTTPS Git 依赖被误认成 Windows 私有路径而失败，资产未上传。当前路径规则已增加盘符左边界，完整 JUnit 的密钥反例参数名也不再泄露测试样本；真实盘符、文件 URI、用户目录和密钥内容仍有直接拒绝回归。本地非 live 2232 项及候选同款证据扫描均通过。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行，当前不能冒充候选资产或稳定 Release。
