...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI。D 发布链实现已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b` 推送；该 SHA 的 CI run `30502065534` 因主窗口失去强引用后触发 pyqtgraph 坐标轴销毁竞态而失败，没有 pytest 断言失败。当前已补齐顶层窗口生命周期锚、图表停表与关闭顺序，新增 3 条红绿回归，最终非 live 为 2227 项通过、0 失败/错误/跳过；修复仍待提交级 CI。提交后 `git archive`、全新 Python 3.12 安装和真实快捷方式桌面矩阵仍未完成。
...
- D 发布链主体已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b` 推送，但同 SHA 的 CI 在首个界面冒烟用例后发生 Qt 底层访问冲突并正确变红。图表重复关闭、主窗口显式关闭和工作区直接销毁 3 条生命周期反例先失败；修复后相关界面套件连续 5 轮各 43 项通过，最终非 live 2227 项通过、0 失败/错误/跳过，工作流数量门同步提高到 2227。当前修复仍是未提交工作区，不能冒充提交级 CI、干净源码安装或稳定 Release。
