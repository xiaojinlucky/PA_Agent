...
- 阶段 A 发现正式 v4 库有 2 组历史授权复用，普通 v5 迁移按设计失败关闭。用户已授权显式安全切换；实现改为两阶段：第一提交先让 WorkerStore 与 ExecutionStore 的全部写入口共同遵守持久跨进程栅栏，正式协议同时锚定控制数据库，并让 GUI 生命周期锁与安装器互斥；所有旧 Worker、Campaign、GUI 退出且 Git HEAD/源码逐文件核对通过后才绑定该提交完整 SHA。第二提交才允许封存 v4、建立空队列 v5 和执行崩溃恢复。第二轮对抗审查已关闭全部 P0/P1，本地非 live 为 2265 项通过、0 失败、0 跳过，尚待提交级 CI；生产数据库仍为 v4 且未修改。
