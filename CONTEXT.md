...
- v0.1.0 正在按 PRD11 推进。B1 Longbridge 合同、B2 无 Qt Controller 和 C 阶段原生 PyQt6 新页均已完成源码与提交级 CI；C 实现提交为 `91715ee2c8af0e6e829031b5a4f375d18f71cf52`，GitHub Actions run `30492305516` 全绿。新页保留旧 OKX Demo 工作台，四市场只读、10m 主周期、1h/4h 可缺失、完整 SHA、行情/日历/认证/切换/分析状态均已接线；下一步是 D 发布证据与安装链，真实桌面矩阵仍须用户从正式快捷方式验收。
...
- C 阶段本地非 live 为 2164 项通过、0 失败、0 错误、0 跳过；远端确定性门收集 2164 项、0 失败、0 错误、1 项既有 UTC 主机条件跳过。16 张离屏图覆盖两种窗口尺寸、三档缩放和九类状态，逐张人工复核并通过尺寸/字号/零运行态读取/零交易控件机器门；图片显示完整 `91715ee...` SHA。对抗审查发现并关闭了旧分析回调覆盖新状态、用 K 线开盘时间误判新鲜度和日历未知夹具不真实三项缺口。真实 Longbridge/Crypto 桌面验收尚未完成，不能把离屏夹具说成桌面成功。
