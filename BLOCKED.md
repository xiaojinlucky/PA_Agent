...
Git，测试会自行构造并确认两个 Git；提交 `c5db9e8` 的 CI 与候选全新安装门已证明
该修复有效。候选随后因 100% 缩放图文件名与验证合同不一致而失败，资产未上传；
当前已显式映射三个缩放名，本机 16 张图及元数据、65 项定向和 2231 项非 live
均通过。提交级 CI 与候选 workflow 仍须绑定将要发布的 SHA 运行，不从上述真实
运行态与桌面阻塞中移除任何一项。
