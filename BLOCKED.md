...
3. **阶段 A 的生产切换尚未执行**：切换器 `2d81d450db780ec6e751ce8ca538dc63df239f6e` 已通过 CI，但生产 `--check` 安全发现其错误要求“全部历史命令数=7”；未改数据库、未激活栅栏。当前修复已把授权准确限定为 2 个复用组、组大小 3 与 4、合计 7 条且全为 `submit`，其他完整合法终态历史只归档不迁入活动 v5；48 项定向测试、全仓非 live 2343 项和独立复审均通过，但修复 SHA 尚未取得 GitHub CI 绿色证据。只有该 SHA 的 CI 绿色后，才能重新读取生产硬门并执行前向可恢复切换。控制库仍为 v4，Worker 停止，风险停止保持；禁止手工 SQL 清空或改写历史租约。本机封存只能称为只读 ACL 加哈希可检测，不能称为 WORM 不可变存储。
