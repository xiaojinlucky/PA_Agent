...
的 CI、候选资产和 16 图逐张复核均通过。随后文档提交
`6b81574013340c162ac9e83e8f3636084f8499dd` 的 CI run `30519576042` 与候选 run
`30520056543` 虽然全绿，但下载复核发现候选包内五层能力索引仍停在旧 SHA，并把
已完成的发布链三层写成未完成；该候选已拒绝。当前本地已增加候选 SHA 绑定、完整
JUnit/全新安装合同、压缩后全文件与索引哈希复核，8 个红色反例全部转绿，最终
非 live 2245 项通过、0 失败，独立复核无 P0/P1/P2。仍须由新 SHA 的 CI、候选下载
与 16 图复核关闭阶段 D；阶段 A 运行态和正式快捷方式桌面阻塞也仍未移除，因此
不得创建稳定 `v0.1.0` tag 或 GitHub Release。
