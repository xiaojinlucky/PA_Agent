...
3. **阶段 A 的两阶段安全切换尚未完成**：用户已授权显式 v4→v5 安全切换。普通迁移仍因 2 组历史 `NEW_RISK` 授权复用、共 7 条不同 `submit` 命令而按设计失败关闭；控制库仍为 v4，Worker 当前停止，没有命令重放、半迁移或新增券商写入。持久栅栏基座与性能修复已分别发布；旧版自动栅栏的一次性受控恢复已完成代码、2295 项本地非 live 测试和无 P0/P1 的对抗复审，但尚未提交，也未取得目标 SHA 的 GitHub CI。只有新 SHA 的 CI 通过后，才能在四重生命周期锁与 SQLite 事务下安装生产协议，再发布并执行下一阶段切换器。禁止手工 SQL 清空或改写历史租约。
