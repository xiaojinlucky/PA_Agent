"""限时 OKX Demo 黄金自动交易实验。

该入口故意固定交易范围, 不读取或改写日常交易路由:

- OKX XAU-USDT-SWAP / 30m
- PA 极度激进
- 执行置信度门槛 30
- OKX Demo XAU-USDT-SWAP / cross / 1 张
- 运行窗口 24 小时
"""
...
def build_campaign_settings(base_settings):
    """复制设置并只在内存中应用实验路由。"""
    ...
    settings.execution.okx.quantity = CAMPAIGN_QUANTITY
...
def validate_campaign_settings(settings) -> None:
    ...
    checks = {
        ...
        "okx.quantity": str(route.quantity) == CAMPAIGN_QUANTITY,
...
class CampaignStateStore:
    ...
    def save(self, state: CampaignState) -> None:
        ...
        try:
            ...
            raise CampaignError(f"无法耐久保存实验状态: {self.path}") from exc
...
def okx_demo_private_preflight() -> dict[str, Any]:
    ...
    instruments = client.instruments("SWAP")
    instrument = next(
        (
            row
    ...
    if minimum > Decimal(CAMPAIGN_QUANTITY) or lot != Decimal("1"):
        raise CampaignError("OKX 黄金永续最小数量或步长已变化, 禁止启动")
    ...
    if max(max_buy, max_sell) < Decimal(CAMPAIGN_QUANTITY):
        raise CampaignError("OKX Demo 可交易数量不足 1 张")
    ...
    if not any(len(row) >= 9 and row[8] == "1" for row in market_rows):
        raise CampaignError("OKX 黄金永续没有可用的已收盘 30m K 线")
    return {
        "simulated": client.simulated,
        "account_identity_present": True,
        "instrument": CAMPAIGN_INSTRUMENT,
        "instrument_state": str(instrument.get("state") or ""),
        "minimum_quantity": str(minimum),
        "quantity_step": str(lot),
        "max_buy_sufficient": max_buy >= Decimal(CAMPAIGN_QUANTITY),
        "max_sell_sufficient": max_sell >= Decimal(CAMPAIGN_QUANTITY),
...
class OkxCampaignSource:
    ...
    def subscribe(self, symbol: str, timeframe: str) -> None:
        if symbol != CAMPAIGN_INSTRUMENT or timeframe != CAMPAIGN_TIMEFRAME:
            raise CampaignError("OKX Demo 实验只允许自身执行产品的 30m K 线")
...
def build_runtime() -> CampaignRuntime:
    base_settings = load_settings(SETTINGS_JSON_PATH)
    settings = build_campaign_settings(base_settings)
...
class OkxDemoCampaign:
    ...
    def process_latest_closed_bar(self) -> bool:
        ...
        if frame is None:
            raise DataSourceTransientError("XAUUSD 30m 已收盘 K 线不足")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="PA_Agent 24 小时 OKX Demo XAUUSD/30m 实验"
    )
    parser.add_argument(
        "command",
        choices=("preflight", "run", "status"),
        help="preflight=私有只读预检; run=实际模拟下单; status=读取状态",
    )
    return parser
