...
> 本节前两条旧实验只作历史审计；现役配置和运行证据以 5.9–5.12 及根目录 `CONTEXT.md` 为准。

- 旧版 5 分钟 Demo 实验、7 笔限价单和生命周期 canary 已有历史证据；它们不能被解释成当前 15 分钟产品配置或实盘绩效。
- 现役本地配置（不代表 Live 授权）为 `OKX Demo / XAU-USDT-SWAP / 10m / extreme_aggressive / min_trade_confidence=20`；进场和主动离场均为 `limit_with_slippage / 0.50 × ATR14`。10m 由两根官方已收盘 5m K 线严格聚合，GUI 已显示并持久化数据来源、Demo 环境、执行品种、模式、ATR 倍数和 1h/4h 薄背景。
...
- 目标套件 `168/168` 通过，包含无活动账户刷新、正确 Demo 路由、陈旧快照、快照字段、内部换币分类和余额变化后旧计划失效。后续 `WO-RUN-06` 已把资金流扫描边界、高水位耐久化及恢复接线部署到生产控制库 v4。最终 Worker PID `48324` 自 `2026-07-24T06:16:53Z` 连续完成对账；其启动首轮一次 `BrokerTransportError` 已触发工单规定的持久 fail-safe 停新险，后续读取恢复也不会自动解除，必须经显式人工 Worker 命令重锚。未实现边界仍是 Funding 账户充值/提现、基线后的非 USDT 外部转账时点换算和七天以上停机补链。
...
- 风险运行态、v1/v2/v3→v4 持锁迁移、资金流分类、七天滚窗、提交前动态定仓、动态杠杆回撤阻断、50% 持久停止和 OKX 合约规格定仓均已通过离线硬验收。生产控制库已从 v1 迁移到 v4；迁移前备份的 SHA-256 为 `1516981E36A8D16411C36326987669C6AAA40AD0BC299636A46B9829E9C392BF`。首轮迁移 Worker PID `49048` 建立可信风险基线，最终审计字段加载后曾重启为 PID `48324`；任何“现役 PID”仍须按当前现场重新探测。历史 uncertain submit 已用账户身份、执行事件和券商空仓空单证据裁决为 `confirmed_not_written_schema_validation`，未解决 uncertain 写入为 0。Campaign `0d239206-c7bc-436c-9944-23e9433e34d5` 的现役状态也必须按进程和状态文件实时复核。

### 5.13 2026-07-24 Demo-S 真实回归与最终有效限价

- 新回归先证明了限价超时语义：execution `622b53fa-6d53-547f-ab4f-bf8ed3e2c9c6` 的 OKX Demo 入场单 `3770752241751908352` 成交 `0`，270 秒后确认撤销并落为 `canceled`，没有改走市价。
- 随后 execution `cf351b86-9f24-59b5-9814-ab203e0fbb19` 经真实监督与唯一 Worker 完成 `111596` 张往返；入场单 `3770781223721459712`、两张原生 OCO、主动离场单 `3770785337595486208` 均有券商回读，执行最终 `closed`、剩余 `0`。最终只读回查为空仓、普通挂单 `0`、八类算法挂单 `0`；这仍只是 Demo 技术验收。
- 真实反馈发现：阶段二信号价、`limit_with_slippage` 最终有效限价和风险数量不能混成一个字段。现役 Campaign 的监督/风险快照明确保存两者；受控可成交输入通过反推信号价，让适配器只应用一次 `0.50×ATR14`，风险定仓和容量查询使用最终有效限价。状态配置的 `price_source` 也改为 `pa_stage2_entry_price_plus_0.50_atr_effective_limit`。
- Demo runner 离场等待从 120 秒改为 330 秒，异常减险命令完成后继续等 Worker 终态；Campaign 只读监控会同步较新的 execution 状态。Codex CLI 版本探测超时已改为临时错误语义，不再误报未安装或让 Campaign 因单根 K 线退出。
- 现役 Campaign 已按新配置指纹重建；外部总控仍必须现场核验其进程、最新 K 线、Worker、租约和券商空仓空单，不能把本文 PID 或 `active` 当永久事实。

### 5.14 2026-07-24 监督数量冻结与重启去重

- 自然与受控监督输入都显式包含信号价、最终有效限价、止损、USDT 风险权益、10% 预算和技术数量；动态杠杆授权以耐久风险快照中的最终有效限价为准。
- Worker 提交前继续刷新账户总权益和资金流水用于回撤，但风险定仓统一使用 USDT 结算币权益。新数量必须与监督数量精确相等，任何变化都阻断旧计划，禁止静默改量。
- Demo-S execution 创建后立即写回 Campaign；返回证据区分监督数量与最终账本数量。异常撤单后出现部分成交时会继续主动离场；Campaign restart 继承最后已处理 K 线，禁止重复分析或重复交易。
- OKX 真实容量实证为 20×=`120000`、25×=`55000`，容量并非随杠杆单调增加；现役规则继续硬阻断并耐久保存触发点，不截断数量。
...
- 最新非 Live 全量共 1510 项：1479 通过、31 失败；31 项与本轮开工前既有清单一致，集中在旧预测 UI、严格/宽松校验、连续性、数据源切换、OpenClaw 和提示音契约。风险、监督、Campaign、Worker、动态杠杆和资金流定向套件没有失败，全量不能描述为全绿。
- 29 个本轮 Python 文件的 Ruff `E9/F`、`compileall` 和 `git diff --check` 通过；本轮只使用用户授权的 OKX Demo，Live 始终禁止。
...
本地执行闭环：用户已授权的 PA Demo 改动在本机验证通过后，由 Codex 按项目 Git 规则列明最终文件快照、验证和安全检查，并在取得当次提交确认后精确暂存、提交、推送 `xiaojinlucky/PA_Agent` 的公开 `main`；外部总控每次只负责读取公开仓库、审核结果并生成下一步工单，不得要求私有仓库权限。

网页版 GPT 不得：

- 使用旧提交、上游仓库、搜索摘要或历史快照替代当前 `main`。
- 把仍在规划中的加仓、反手、父级生命周期聚合或完整暂停/紧急清仓写成当前能力；动态杠杆只能按已部署的 Demo 首仓有界授权链描述，不能扩写成多仓位能力。
