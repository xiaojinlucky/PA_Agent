> **落库说明（2026-07-23）**：本文件是外部规划输入的原文归档，设计正文基于旧基线 `3ae95321eef706889cf901c63a1c293f11f7608d`。当前能力事实以当前代码、`CONTEXT.md` 和阶段基线记录为准；目标行为和产品要求以本 PRD 中已确认的用户决策为准；当前代码尚未实现的目标需求应标记为目标态，不得因此删除或视为无效。本文件是工程目标与硬验收输入，不表示功能已实现，也不构成交易授权。除本说明外，正文保持原文。
