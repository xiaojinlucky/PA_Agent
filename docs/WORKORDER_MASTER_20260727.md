...
- [ ] v0.1.0 阶段 D 发布链：主体已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b`
  ...
  Git 依赖误认成 Windows 私有盘符路径，资产未上传。当前盘符边界与完整 JUnit 参数
  脱敏回归均先红后绿，66 项定向、2232 项非 live 和候选同款证据目录扫描均通过。
  仍须让证据扫描修复对应发布 SHA 通过提交级 CI 与候选 workflow。阶段 A 私有真相和正式快捷方式桌面矩阵未通过前，
