...
- [ ] 阶段 A 显式安全切换：`2d81d450db780ec6e751ce8ca538dc63df239f6e`
  已通过 CI，但生产 `--check` 安全发现其把“2 组中合计 7 条异常复用命令”误写成
  “全部历史命令只能有 7 条”，因此未改数据库。当前修复只冻结异常组为 3+4、
  合计 7 条且全为 `submit`，其他完整合法终态历史只封存不迁入活动 v5；intent、
  active 栅栏、只读封存、正式 v5 结构、唯一回执、完整表内容对照、completion 与
  前向恢复继续为单一合同。48 项切换测试和全仓非 live 2343 项通过、0 失败/错误/
  跳过；独立复审 P0/P1/P2 均无。当前仍须取得修复 SHA 的绿色 CI，再重新执行
  生产安全硬门并完成切换/Worker/受控 OKX Demo 验收。
  本机档案只承诺只读 ACL 与哈希可检测，不承诺断电级目录持久性或 WORM。
