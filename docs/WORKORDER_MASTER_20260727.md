...
- [ ] 阶段 A 显式安全切换：本地候选已只接受精确 7 条历史命令和 2 组历史租约
  复用，并把最终源核验、intent、active 栅栏、只读封存、正式 v5 结构、唯一回执、
  完整表内容对照、completion 与前向崩溃恢复连成单一合同。45 项切换测试和全仓
  非 live 2340 项通过、0 失败/错误/跳过；独立复审无 P0/P1。当前仍须取得目标
  SHA 的绿色 CI，再重新执行生产安全硬门并完成切换/Worker/受控 OKX Demo 验收。
