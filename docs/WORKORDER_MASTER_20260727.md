...
| 历史 UNCERTAIN 命令 | 已有耐久处置证据，禁止重放；具体编号只保留在本地账本和 Git 历史 |
| 固定风险参数 | `20000 USDT 上限 / 10% / 20x / risk_budget` |

### 0.4 行为红线

- 固定代理失败必须闭锁新增风险（fail-closed）；**永不**回退直连、系统代理或用户日常代理。OKX 流量只走项目专用的固定本机代理通道。
...
- 本文后文记录的旧进程、Campaign、账户和订单状态都是历史证据，不能当成当前事实。
...
**验证**：Campaign 定向 86 项通过、0 失败；三套件 1886 项通过、0 失败。两轮独立对抗审查最终 PASS。受控恢复完成新的账户与账单读取，临时风险停止按白名单原因合法恢复，高水位未重锚；首根新 K 线完成且零新增 execution、订单或仓位。
...
- [x] 2026-07-27 无人值守运行证据：Campaign 完成 **50 根自然 K 线、0 失败**；
  一次自然信号经 Worker 提交真实 OKX Demo 限价单，等待窗口内未成交后按规则正常撤销，
  没有切换成市价单，形成完整自然交易生命周期实证。当时同时发现 P2：
  该笔交易的 NEW_RISK 租约在交易终态后仍被 Controller 续期；根因与修复见本账本后续
  “NEW_RISK 最小权限收口”条目。
- [x] 2026-07-27 WO-C 完成：硬门核验后停旧树、租约自然过期并按 restart 语义重启，
  新 Campaign 正确继承 `last_completed_bar_ms`，首根自然 K 线完成（no_order，0 失败）；
...
- [x] 2026-07-28 WO-E 外部视觉探索：用户完成附件提交并直接否决 Stitch 结果，该稿已作废且不进入实现；后续 v0.1.0 工单明确以 PRD11 为最终规格。官方合同确认 Longbridge 批量报价单次最多 500 个标的；OKX 自选可按 SPOT/SWAP 最多两个串行 ticker 快照。10m 最坏 602 根 5m 需要最多三页，现有客户端缺 `after`；正确修复需极窄解除 `pa_agent/execution/okx_client.py` 禁区，未用削减窗口或私有调用绕过
...
- [ ] v0.1.0 阶段 D 候选发布链：主体已由 `4c9d16fe29611a2c35eaa725e2e843d2e745885b`
  ...
  非 live 全量均通过，工作流数量门同步为 2229。提交 `e835404` 的 CI 和候选源码归档门
  已通过；候选全新安装又暴露 PATH 中多个 `git.exe` 被合成一个命令。当前已选择首个 Git，
  回归已自行构造并确认两个 Git，独立审查发现的非确定性 P2 已关闭；提交 `c5db9e8` 的
  CI 和候选全新安装门均已通过。缩放命名修复提交 `151be1d` 的 CI 已全绿，候选
  `30510973235` 也已通过全新安装和 16 张离屏证据生成；随后证据扫描器把固定 HTTPS
  Git 依赖误认成 Windows 私有盘符路径，资产未上传。盘符边界修复提交 `025becf`
  的 CI 已全绿；下载证据复核又发现 JUnit 跳过文本共含 8 处 Runner 绝对路径，
  因此未触发候选。JUnit 脱敏提交 `6bdc3b2` 的 CI 与候选现均全绿，四个资产已下载
  并通过机器合同；稳定复核继续拦下两个误绿：源码 ZIP 的 565 个条目内容相同但时间戳
  随主机时区相差 8 小时，且 Runner 缺中文字体导致下载图大量方框。当前归档子进程
  固定 UTC；候选固定并校验 Noto 中文字体，截图和验证器共同要求核心中文覆盖。修复
  提交 `4f9543478b1bac12d293fd3a8b3296180e5cfbd7` 的 CI `30516280889` 与候选
  `30516712766` 均全绿；2235/0/0/1 JUnit、四资产、上海时区逐字节重建和全新安装
  通过。但独立下载图审查发现固定字体不支持 `↻`，加载与分析中仍显示带叉方框，
  旧机器门误绿，因此该候选已拒绝。修复提交
  `c509516a7c57469d49995e95f36a060fe5667ca4` 改用受支持的 `…`，并将全部界面必需
  符号纳入字体与元数据硬门；其 CI `30518459269` 和候选 `30518850754` 均全绿，
  最终非 live 2237 项通过、0 失败。四资产、跨时区逐字节重建、全新安装和扫描通过，
  下载的 16 张状态/尺寸/缩放图已逐张确认没有缺字、截断或重叠。后续文档提交
  `6b81574013340c162ac9e83e8f3636084f8499dd` 的 CI `30519576042` 和候选
  `30520056543` 虽然全绿，但下载包内五层能力索引仍停在旧 SHA，并保留发布链三层
  非终态，故该候选已拒绝。索引修复提交
  `7a3297fbae053aa6e75f35bead322ba71afa045a` 的 CI `30522965861` 与候选
  `30523309766` 全绿，下载复核证明索引、压缩包、全新安装、扫描和跨时区重建均已
  绑定当前 SHA；但 `calendar_unknown` PNG 仍画着初始化的“连续交易”，而元数据写
  “状态未知”，候选再次拒绝。当前本地先完成焦点遍历和最后一次 Qt 事件处理，再
  同步渲染控件树；截图后元数据保持纯读取，并将 PNG 哈希写入元数据硬门。候选 ZIP
  压缩后重跑完整 16 图合同；5 个红色反例、66 项发布定向、本地
  16 图矩阵和最终非 live 2248 项均通过、0 失败，数量门提高到 2248。新 SHA 的 CI、
  候选和下载复核，以及阶段 A 私有真相和正式快捷方式桌面矩阵未通过前，禁止创建
  稳定 `v0.1.0` tag 或 GitHub Release。
- [x] 阶段 A 正式栅栏协议：`bef8e986787d8c5055af32a63516859365a6239e`
  已推送，CI run `30549842331` 全绿；生产协议已安装为 generation 0/inactive，
  Worker 保持停止，控制库仍为 v4，风险停止未解除。
- [x] 阶段 A 显式安全切换：历史复用范围修复由
  `0cfd5ca4c9c830bb543a4c61e0920ce9d41d22bc` 发布，CI run `30567235969`
  全绿。新鲜私有真相和本地安全门通过后，正式计划摘要
  `e372f38e29cea1bbdf55ee44ea1447f34929bf6f75ffe3f546aa0fa259fbe37f`
  的 v4→v5 切换完成；档案、schema v5、空队列、Worker 心跳和只读对账通过。
  迁移没有自动清除风险停止。本机档案只承诺只读 ACL 与哈希可检测，不承诺断电级
  目录持久性或 WORM。
- [ ] 阶段 A 受控 OKX Demo：首次入场被确定拒绝，券商和本地最终均为空，无未解决
  `UNCERTAIN`。客户端只保留了顶层“全部操作失败”，没有保存逐单 `sCode/sMsg`。
  当前修复要求全失败保留真实逐单拒绝原因，任何成功项或部分成功一律强制对账，
  畸形明细不能遮住成功证据，响应/请求正文/查询参数中的订单标识在异常入账前
  脱敏，`sCode` 统一规范化；14 条红灯已转绿，另有 6 个错误响应/兼容性场景通过。
  相关回归 388 项和最终非 live 2363 项均通过、0 失败，独立复审 P0/P1/P2 均无；
  修复 `90782ab71f3742f946eead4f9c28ebc5021e4ea7` 的 CI run `30571509003`
  已全绿。重载前又发现 50001 临时服务故障没有进入受控恢复白名单；官方合同和
  新鲜私有空现场已核实，红灯转绿后最终非 live 2366 项通过、0 失败。新修复的
  提交级 CI 和新 SHA Worker 重载未通过前禁止恢复停止或重试；最终仍须完成
  入场—成交—原生保护—主动离场—CLOSED，并复核空仓空普通单和空八类算法单。

## 6. 用户侧待办（只有用户能做）

1. **密钥轮换（长期提醒，未完成）**：历史本地会话材料曾包含券商和模型凭据；服务端轮换只能由用户本人操作，轮换后同步仓库外的共享凭据文件。当前 Longbridge 综合行情档案已真实可用，不要求为本工单重签。
