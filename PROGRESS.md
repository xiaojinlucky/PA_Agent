...
- Worker 当前保持停止，风险停止耐久状态仍保存在完整的 v4 控制库。正常 v5 迁移明确禁止静默挑选、清空或改写历史租约，因此不能用手工 SQL 绕过。继续发布需要用户在“保持 v4 原库为不可变审计档案并做显式 v5 安全切换”与“停止稳定 Release”之间作出新的数据库处置授权；这超出此前仅授权普通 v4→v5 迁移的范围。
