...
- 本地验证：发布合同 52 项通过；扩展定向 305 项通过；最终 `-m "not live"` 为 2224 项通过、0 失败、0 错误、0 跳过，另有 7 项真实联网用例按门未运行。两个入口自检均通过，37 个提示词资源、两个入口、Python 3.12 和交易默认关闭均得到机器验证。
- 当前仍未提交。下一步是精确暂存、安全扫描、提交并等待该 SHA 的 CI 和候选 workflow；随后从该提交生成 `git archive`，在全新目录和全新 Python 3.12 虚拟环境完成安装/卸载验收。阶段 A 与正式快捷方式桌面矩阵仍是稳定 Release 硬阻塞。
