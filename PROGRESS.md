...
- 整改改为两个独立提交。第一提交只部署持久写入栅栏：两个 Store 的全部生产写入口共享同一跨进程锁、状态、初始化标记和代际；正式协议同时写入控制数据库，外部状态与标记同时丢失也会失败关闭；GUI 从启动到退出持续持有写入锁，安装器在同一把锁内复核旧 GUI、Worker、Campaign、Git HEAD、受跟踪文件状态、四份协议源码逐文件内容和已加载栅栏源码，再绑定完整提交 SHA。两轮对抗审查复现的 WAL 回执漏读、正式协议静默重建、GUI/提交核对竞态、Git 隐藏工作区改动和 marker-only 安装中断均已修复；第二轮没有剩余 P0/P1，17 个直接安全测试覆盖官方安装器的 GUI 锁拒绝路径，最终非 live 为 2265 项通过、0 失败、0 跳过。第二提交将实现并验证 v4 封存、空队列 v5、旧写入者终检和硬崩溃恢复；第一提交的远端 CI 与正式协议安装未完成前，禁止生产切换。
