...
交易执行不会随安装自动开启。主窗口的一级页签 **实盘交易** 是 OKX Demo 工作台；菜单中的同名入口只负责切到该页，不再打开配置弹窗。默认的风险预算模式由用户设置资金上限、单笔风险比例和最大允许杠杆，系统按真实止损距离、合约规格与实时容量唯一计算张数；固定张数模式由用户设置合约张数和最大杠杆，系统反算最坏损失，超过资金或容量上限时直接阻断，绝不静默减量。密钥只从仓库外的 `Quant\env` 读取。真实写操作还必须显式开启共享总开关、券商专用开关（OKX Live）并在每次启动后输入确认文字。桌面正式入口为 `D:\Desktop\PA_Agent.lnk`；工作台分开显示 Campaign 当前冻结参数和已保存的下次启动参数，两者不一致或状态不新鲜时会标红。完整流程与限制见操作文档。
...
OKX 网络固定走 PA Agent 独占的本机代理 `127.0.0.1:10981`，不会跟随日常 v2rayN `10808` 的当前节点，也不会被系统 `NO_PROXY` 绕过。工作台会显示实际激活节点的运行元数据；固定代理不可用时系统明确失败并闭锁新增风险。该代理目前随当前 Windows 用户登录启动，不代表无人登录时也能跨重启持续运行。
