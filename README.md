...
面向主观交易者的 **价格行为（Price Action）** AI 辅助决策工具。从 **MT5 / TradingView / Longbridge / OKX / yfinance / AkShare** 读取 K 线，将结构化 K 线数据与预计算特征送入大模型做**两阶段分析**（市场诊断 → 交易决策），**不是**截图识图。v0.1.0 新增加密、美股、港股、A股四市场只读看盘页；交易默认关闭，本版本只保留受控 OKX Demo 安全工作台，明确不发布 OKX Live 或 Longbridge 交易能力。
...
- 🖥️ **四市场只读看盘**：加密、美股、港股、A股；10m 为主周期，1h/4h 为可缺失背景
...
- 🔒 **默认关闭的 OKX Demo 工作台**：覆盖入场、部分成交、保护、主动离场与重启对账；v0.1.0 不开放 OKX Live 或 Longbridge 交易
...
v0.1.0 是源码部署版。推荐从 GitHub Release 下载源码 ZIP、核对 `SHA256SUMS` 后，按 [Windows 源码安装说明](docs/SOURCE_INSTALL_WINDOWS.md) 安装。
...
交易执行不会随安装自动开启。主窗口的一级页签 **实盘交易** 是 OKX Demo 工作台；菜单中的同名入口只负责切到该页。默认风险预算模式按资金上限、单笔风险比例、止损距离、合约规格和实时容量唯一计算数量；固定数量模式若超过资金或容量上限会直接阻断，不会静默减量。v0.1.0 只发布 OKX Demo 安全工作台，OKX Live 与 Longbridge 交易属于明确排除项。

公共执行层使用 Worker schema v5 的一次性新增风险授权：一次授权在数据库中只允许绑定一条提交或调整杠杆命令，减仓、撤单等降低风险的命令不受影响。旧控制库只由持有唯一 Worker 锁的进程迁移；若旧库已存在同一授权对应多条新增风险命令，迁移会原样停止并要求人工审查，不会自动挑选或删除历史记录。

OKX 网络使用 PA Agent 独占的固定本机代理，不跟随用户日常代理节点，也不会被系统代理变量静默绕过。固定代理不可用时系统明确失败并闭锁新增风险，不会回退直连。
